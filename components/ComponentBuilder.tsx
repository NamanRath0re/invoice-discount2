...
//     },
//     // formUi = what the form renderer shows the user (visible / editable)
//     formUi: {
...
//     },
//     // Default formUi for new fields is visible + editable
...
//  * ALL fields (edited + untouched) are serialised so the backend
//  * receives the complete rendered_json every time.
//  */
...
//     // formUi – visible / editable for the renderer
//     // Only emit if not both default (true/true) to keep payload clean
...
//     hash_key: hashKey,
//     version,
//     rendered_json: { fields, step_key: stepKey },
//   };
// }
...
// export default function EnhancedComponentBuilder({ formId, stepKey }: ComponentBuilderProps) {
...
//     link.setAttribute('download', `form-schema-${Date.now()}.json`);
//     link.click();
//   };

//   /** Publish – serialise ALL fields and call update API */
//   const handlePublish = async () => {
//     if (!formId || !stepKey || !stepMeta) {
//       alert('Missing formId / stepKey / stepMeta – cannot save.');
//       return;
//     }
...
//         stepMeta.version
//       );
//       console.log('[FormBuilder] Update payload:', JSON.stringify(payload, null, 2));

//       // TODO: replace URL with actual update endpoint
...
//     } catch (e) {
//       console.error('[FormBuilder] Publish error', e);
//       setSaveStatus('error');
...
//     if (dtError) {
//       return (
//         <div className="flex flex-col items-center py-6 text-red-500 text-sm gap-2">
...
//     // Drill-down: show field keys for selected type
//     if (selectedDataType) {
//       return (
...
//             </div>
//           ) : fkError ? (
//             <div className="flex flex-col items-center py-4 text-red-500 text-xs gap-1">
//               <AlertCircle className="h-4 w-4" />
//               <span>Failed to load fields</span>
//             </div>
//           ) : fieldKeys.length === 0 ? (
//             <p className="text-xs text-gray-400 py-4 text-center">No fields available</p>
//           ) : (
//             fieldKeys.map((item) => {
...
//                     <span className="text-xs text-gray-400 truncate">{item.field_key}</span>
...
//               <div className="p-1.5 bg-gray-100 rounded shrink-0">
//                 {DATA_TYPE_ICONS[dt] ?? <Type className="h-4 w-4" />}
//               </div>
//               <div className="text-left flex-1 min-w-0">
//                 <div className="font-medium text-sm capitalize">{dt}</div>
//                 <div className="text-xs text-gray-400">Browse fields →</div>
//               </div>
//             </div>
//           </Button>
//         ))}
...
//       {/* ── Sticky header ──────────────────────────────────────────────── */}
//       <header className="sticky top-0 z-20 bg-white border-b shadow-sm px-4 sm:px-6 py-3">
...
//               {stepKey ? (
//                 <>
//                   Step: <span className="font-semibold text-gray-700">{stepKey}</span>
//                   {formId && <span className="ml-2 text-gray-400">(form #{formId})</span>}
//                 </>
//               ) : 'Declarative UI + Rules + Effects'}
//             </p>
//           </div>
...
//             )}
//             {saveStatus === 'error' && (
//               <span className="text-xs text-red-600 bg-red-50 border border-red-200 px-2 py-1 rounded font-medium">
//                 ✗ Save failed
//               </span>
//             )}
...
//             </Button>
//             <Button size="sm" onClick={handlePublish} disabled={saveStatus === 'saving'}>
//               {saveStatus === 'saving'
...
//         {stepLoading && (
//           <div className="mt-2 flex items-center gap-2 text-xs text-blue-600 bg-blue-50 px-3 py-1.5 rounded w-fit">
//             <Loader2 className="h-3 w-3 animate-spin" /> Loading step fields…
//           </div>
//         )}
//         {stepError && (
//           <div className="mt-2 flex items-center gap-2 text-xs text-amber-700 bg-amber-50 px-3 py-1.5 rounded w-fit">
...
//           {/* ── LEFT ─── Component Library + API Endpoints ──────────── */}
//           <div className="w-full xl:w-72 2xl:w-80 shrink-0 flex flex-col gap-4
//                           xl:sticky xl:top-[69px] xl:max-h-[calc(100vh-80px)] xl:overflow-y-auto">
...
//                   <Globe className="h-4 w-4" /> API Endpoints
//                 </CardTitle>
//               </CardHeader>
//               <CardContent className="px-4 pb-4 pt-0">
//                 <Button variant="outline" className="w-full mb-3 text-sm h-9" onClick={handleAddApi}>
//                   <Link className="h-4 w-4 mr-2" /> Add Endpoint
//                 </Button>
//                 <div className="space-y-2">
//                   {schema.apis?.map((api) => (
//                     <div
//                       key={api.id}
//                       className={`p-2.5 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors ${
//                         selectedApi?.id === api.id ? 'border-blue-500 bg-blue-50' : ''
//                       }`}
//                       onClick={() => { setSelectedApi(api); setActiveTab('apis'); }}
//                     >
...
//                         }`}>{api.method}</span>
//                         <span className="text-xs font-medium truncate flex-1">{api.name}</span>
//                         <Badge variant="outline" className="text-[10px] px-1 shrink-0">
//                           {api.triggers?.onLoad ? 'Auto' : 'Manual'}
//                         </Badge>
//                       </div>
//                       <p className="text-[10px] text-gray-400 truncate mt-1">{api.url}</p>
//                     </div>
//                   ))}
//                 </div>
//               </CardContent>
...
//           {/* ── CENTER ── Canvas / API Playground / Preview ─────────── */}
...
//                     </TabsTrigger>
//                     <TabsTrigger value="apis" className="flex items-center gap-1.5 text-xs sm:text-sm flex-1">
//                       <Globe className="h-3.5 w-3.5" />
//                       APIs
//                       <Badge variant="secondary" className="ml-1 text-xs">{schema.apis?.length || 0}</Badge>
//                     </TabsTrigger>
...
//                     <GripVertical className="h-10 w-10 mx-auto mb-3 opacity-50" />
...
//   /** Save – POST to updateFormStep then call onBack to return to sections */
//   const handleSave = async () => {
//     if (!formId || !stepKey) {
//       alert('Missing formId or stepKey – cannot save.');
//       return;
//     }
//     setSaveStatus('saving');
//     try {
...
//       setSaveStatus('saved');
//       // Small delay so the user sees the ✓ Saved confirmation before navigating back
//       setTimeout(() => {
//         setSaveStatus('idle');
//         onBack?.();
//       }, 800);
...
//                 {stepName || stepKey || 'Step Builder'}
//               </h1>
//               <p className="text-gray-400 text-xs truncate">
//                 {stepKey && <span className="font-mono">{stepKey}</span>}
//                 {formId && <span className="ml-2">· form #{formId}</span>}
//               </p>
//             </div>
//           </div>
