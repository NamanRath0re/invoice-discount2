...
//   endpoint:         string          // e.g. "/kyc/pincode"
//   response_mapping: Record<string, string>  // { city: "city", state: "state" }
// }
...
// function isFieldVisible(field: FormField, values: Record<string, string>): boolean {
//   // Respect ui.visible if present
//   if (field.ui && !field.ui.visible) return false
//   if (!field.conditions?.length) return true
//   return field.conditions.every((c) => {
//     const actual = values[c.depends_on] ?? ""
//     return c.operator === "equals" ? actual === c.value : actual !== c.value
...
//     if (!re.test(value)) return `Invalid ${field.label.toLowerCase()}`
//   }
//   if (value && field.validation?.max_length && value.length > field.validation.max_length) {
...
// function Stepper({ totalSteps, currentOrder, currentName }: {
//   totalSteps: number; currentOrder: number; currentName: string
// }) {
//   return (
//     <div className="w-full">
...
//         {Array.from({ length: totalSteps }, (_, i) => {
//           const n = i + 1
//           const isDone = n < currentOrder
//           const isActive = n === currentOrder
//           const isLast = n === totalSteps
//           const label = n === 1 ? "Mobile" : isActive ? currentName : `Step ${n}`
//           return (
//             <div key={i} className="flex flex-1 flex-col items-center">
//               <div className="flex items-center w-full">
...
//               </div>
//               <span className={cn(
//                 "mt-1.5 text-center text-[10px] leading-tight max-w-[64px] px-0.5",
...
//         className={cn(
//           "h-11 w-full appearance-none rounded-xl border border-input bg-background",
//           "pl-3 pr-10 text-sm transition-colors cursor-pointer",
//           "focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary",
//           "disabled:cursor-not-allowed disabled:opacity-50",
//           !value ? "text-muted-foreground" : "text-foreground",
//           error && "border-destructive focus:ring-destructive"
...
// // ─── Field with data_source (e.g. pincode auto-fill) ─────────────────────────
...
//   // Validate field format before calling API
//   const isReadyToFetch = (v: string): boolean => {
//     if (!field.validation?.regex) return v.length > 0
//     return new RegExp(field.validation.regex).test(v)
//   }

//   const fetchDataSource = useCallback(async (val: string) => {
//     if (!isReadyToFetch(val)) { setFilled(false); setFetchError(""); return }
//     setFetching(true); setFetchError("")
//     try {
//       const endpoint = `${BASE}${ds.endpoint}`
...
//       // Map response fields according to response_mapping
//       const autoFilled: Record<string, string> = {}
//       for (const [sourceKey, targetKey] of Object.entries(ds.response_mapping)) {
//         const responseValue = data.data?.[sourceKey] ?? ""
//         autoFilled[targetKey] = String(responseValue)
//       }
//       onAutoFill(autoFilled)
//       setFilled(true)
//     } catch (e) {
//       setFetchError(e instanceof Error ? e.message : "Lookup failed")
//       setFilled(false)
//     } finally { setFetching(false) }
//   }, [ds, field.key])

//   const handleChange = (v: string) => {
//     // Respect max_length from validation
...
//     debounceRef.current = setTimeout(() => fetchDataSource(capped), 600)
//   }
...
// function DynamicField({ field, value, onChange, onAutoFill, disabled, error }: {
//   field:      FormField
//   value:      string
//   onChange:   (v: string) => void
//   onAutoFill: (mapping: Record<string, string>) => void
...
// export function KycFormStep({
//   customerId, userType, stepKey, totalSteps, onBack, onComplete,
// }: KycFormStepProps) {
//   const [formData, setFormData]       = useState<KycFormData | null>(null)
//   const [values, setValues]           = useState<Record<string, string>>({})
//   const [errors, setErrors]           = useState<Record<string, string>>({})
...
//         method: "POST", headers: API_HEADERS,
//         body: JSON.stringify({
//           form_type: "onboarding", actor: userType,
//           product_code: "SCF", scheme_code: "DEFAULT",
//           customer_id: customerId, step_key: stepKey,
//         }),
//       })
//       const data = await res.json()
//       if (!data.success || !data.data?.[0]) throw new Error(data.message ?? "Failed to load form")
...
//   // Auto-fill multiple fields from a data_source response_mapping
//   const handleAutoFill = (mapping: Record<string, string>) => {
//     setValues((prev) => ({ ...prev, ...mapping }))
//     // Clear errors for auto-filled fields
//     setErrors((prev) => {
//       const next = { ...prev }
//       Object.keys(mapping).forEach((k) => { delete next[k] })
//       return next
//     })
//   }
...
//     try {
//       // TODO: replace stub with real submit API when backend provides it
//       // const res  = await fetch(`${BASE}/kyc/formSubmission2`, { method: "POST", headers: API_HEADERS,
//       //   body: JSON.stringify({ form_id: formData.form_id, submission_id: formData.submission_id,
//       //     customer_id: customerId, step_key: stepKey, form_data: payload }) })
//       // const data = await res.json()
//       // if (!data.success) throw new Error(data.message ?? "Submission failed")
//       await new Promise((r) => setTimeout(r, 600))
//       onComplete(formData.submission_id, formData.step.next_step_key)
...
//   // textarea and full-row fields span 2 cols
//   const isFullWidth = (f: FormField) => f.type === "textarea"
...
//         {/* Left sidebar — vertical stepper (desktop) */}
//         <div className="hidden lg:flex flex-col w-72 shrink-0 border-r px-8 py-10 bg-muted/20
...
//               const isLast = n === totalFromApi
//               const label = n === 1 ? "Mobile Verification" : isActive ? stepName : `Step ${n}`
//               return (
//                 <div key={i} className="flex gap-4">
//                   <div className="flex flex-col items-center">
//                     <div className={cn(
...
//                       {isDone ? <Check className="h-3.5 w-3.5" /> : n}
//                     </div>
//                     {!isLast && <div className={cn("w-0.5 flex-1 min-h-[2rem] mt-1 mb-1 rounded", isDone ? "bg-primary" : "bg-border")} />}
//                   </div>
//                   <div className={cn("pb-8", isLast && "pb-0")}>
//                     <p className={cn("text-sm font-medium leading-none pt-1.5",
//                       isActive ? "text-foreground" : isDone ? "text-primary/70" : "text-muted-foreground")}>
//                       {label}
//                     </p>
//                     {isActive && <p className="text-xs text-muted-foreground mt-1">In progress</p>}
//                     {isDone  && <p className="text-xs text-primary/60 mt-1">Completed</p>}
//                   </div>
//                 </div>
//               )
//             })}
...
//             <div className="lg:hidden mb-8">
//               <Stepper totalSteps={totalFromApi} currentOrder={stepOrder} currentName={stepName} />
//             </div>
...
// function FileField({ field, onUploadComplete, disabled, error }: {
//   field:            FormField
...
//   const up = field.upload

//   const handleFile = async (f: File) => {
//     setFile(f); setUploaded(false); setUploadErr("")
//     if (!up) { onUploadComplete(field.key, f.name); return }  // no upload endpoint — just store name

//     setUploading(true)
//     try {
//       const form = new FormData()
//       form.append(up.field_name ?? "file", f)
//       const res = await fetch(`${BASE}${up.endpoint}`, {
//         method:  up.method ?? "POST",
//         headers: { "X-Tenant-Code": "demo" },  // no Content-Type — let browser set multipart boundary
//         body: form,
//       })
//       const data = await res.json()
//       if (!data.success) throw new Error(data.message ?? "Upload failed")
//       const fileUrl = data.data?.url ?? data.data?.file_url ?? f.name
//       onUploadComplete(field.key, String(fileUrl))
//       setUploaded(true)
//     } catch (e) {
...
//           className={cn(
//             "w-full h-24 rounded-xl border-2 border-dashed flex flex-col items-center justify-center gap-2",
...
//           <span>Click to upload {field.label.toLowerCase()}</span>
//           {up?.accept && <span className="text-xs opacity-60">{up.accept}</span>}
//         </button>
//       ) : (
//         <div className={cn(
...
//         )}>
//           <FileText className={cn("h-5 w-5 shrink-0", uploaded ? "text-green-600" : "text-muted-foreground")} />
...
//           {!uploading && (
//             <button type="button" onClick={() => { setFile(null); setUploaded(false); setUploadErr("") }}
...
// function DynamicField({ field, value, onChange, onAutoFill, onUploadComplete, disabled, error }: {
...
//       {field.type === "file" ? (
//         <FileField field={field} onUploadComplete={onUploadComplete} disabled={disabled} error={error} />
//       ) : field.type === "textarea" ? (
...
//         try {
//           const prevRes  = await fetch(`https://192.168.6.6/2013/api/v1/kyc/getFormDetails`, {
//             method: "POST", headers: API_HEADERS,
//             body: JSON.stringify({ submission_id: submissionId, step_key: stepKey }),
//           })
//           console.log("Previous submission details response:", prevRes)
//           const prevData = await prevRes.json()
//           console.log("Previous submission details:", prevData)   
